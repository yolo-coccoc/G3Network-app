...
MVP lý tưởng cố định thứ tự message và loại bỏ reliability branching. Mỗi
operation chỉ validate dữ liệu cơ bản, ghi history và cập nhật aggregate; caller
ở entry boundary vẫn sở hữu commit/rollback transaction.
"""
...
async def ingest_transaction_event(
    ...
    *,
    ...
    Args:
        ...
        station_id: Station đã resolve từ OCPP identity.
        evse_id: EVSE đã resolve từ OCPP payload.
        connector_id: Connector đã resolve từ OCPP payload.
        transaction_id: Transaction identity do trụ cấp.
        event_type: Một trong Started, Updated hoặc Ended.
        event_occurred_at: Thời điểm event phát sinh.
        meter_start_wh: Meter đầu phiên, thường gửi ở Started.
        meter_end_wh: Meter hiện tại/cuối phiên nếu event có kèm.

    Returns:
        Kết quả accepted chứa session UUID và status hiện tại.

    Raises:
        ChargingSessionInputError: Input không hợp lệ hoặc event sai thứ tự.
        ChargingSessionNotFoundError: Updated/Ended không có Started trước đó.
    """
...
async def ingest_meter_values(
    ...
    *,
    ...
    MVP giả định samples được sắp đúng thứ tự và không lặp. Vì vậy không query
    sample cũ, không tạo fingerprint, không retry và không có outcome duplicate.
    """
...
# Các function cũ được cố ý comment thay vì gọi lại:
# _normalize_meter(...)                 # unit/measurand/phase normalization
# _event_matches(...)                   # duplicate fingerprint
# _mark_meter_reconciliation(...)       # meter conflict/reset handling
# _set_last_event(...)                  # ordering cache
# ingest_transaction_event(... seq_no, received_at, idempotency_key, raw_payload)
# ingest_meter_values(... transaction_id, received_at, idempotency handling)
# mark_station_interrupted(...)         # offline/reconnect interruption
# Các nhánh DUPLICATE, CONFLICT, IGNORED_OUT_OF_ORDER và retry đều thuộc
# reliability path trong future.md mục 27.
