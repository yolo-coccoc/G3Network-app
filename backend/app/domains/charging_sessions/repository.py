...
Repository chỉ truy vấn, tạo và flush aggregate/history. Các helper cũ cho
logical idempotency, ordering, interruption và reconciliation được comment ở
cuối module vì không thuộc điều kiện lý tưởng của MVP.
"""
...
# Các helper dưới đây thuộc planner cũ và đã được cố ý vô hiệu hóa:
# find_event_by_logical_key(...)       # duplicate/seqNo detection
# find_meter_by_logical_identity(...)  # meter idempotency/conflict
# get_latest_meter(..., for_update=True)  # ordering lock
# list_interruptible_sessions(...)     # offline interruption
# Các tham số for_update, idempotency_key, received_at và raw payload cũng được
# bỏ khỏi public repository contract của MVP lý tưởng.
