...
from app.domains.vehicles.service import find_active_vehicle_by_id, find_active_vehicle_by_vin


async def _response(db: AsyncSession, item) -> TelematicResponse:
...
async def create_telematic(db: AsyncSession, data: TelematicCreate) -> TelematicResponse:
    ...
    if data.vehicle_vin:
        ...
        if vehicle_id and await db.scalar(select(Telematic.telematic_id).where(Telematic.vehicle_id == vehicle_id, Telematic.deleted_at.is_(None))):
            raise TelematicConflictError("Xe đã được gán cho telematic khác")
    try:
        item = await repository.create(db, {"telematic_serial": data.telematic_serial, "vehicle_id": vehicle_id, "status": data.status, "firmware_version": data.firmware_version})
...
async def list_telematics(db: AsyncSession, page: int, page_size: int, status: TelematicStatus | None) -> TelematicListResponse:
    """Lấy danh sách thiết bị có phân trang."""
    page_size = min(max(page_size, 1), 100)
    page = max(page, 1)
    items = await repository.list_items(db, (page - 1) * page_size, page_size, status)
    return TelematicListResponse(items=[await _response(db, item) for item in items], total=await repository.count_items(db, status), page=page, page_size=page_size)


async def update_telematic(db: AsyncSession, telematic_id: UUID, data: TelematicUpdate) -> TelematicResponse:
    """Cập nhật thiết bị và resolve lại VIN khi field được gửi."""
    item = await repository.get_by_id(db, telematic_id)
    if not item:
        raise TelematicNotFoundError("Không tìm thấy telematic")
    values = data.model_dump(exclude_unset=True)
    if "telematic_serial" in values and values["telematic_serial"] != item.telematic_serial and await repository.get_by_serial(db, values["telematic_serial"]):
        raise TelematicConflictError("Telematic serial đã tồn tại")
    ...
    values = {key: value for key, value in values.items() if value is not None or key == "vehicle_id"}
    try:
