"""
Runtime entrypoint của telemetry ingestion process.

Module lắp ráp database startup probe, MQTT consumer, batch worker và HTTP health
server trong cùng một asyncio event loop. Entrypoint là component duy nhất sở hữu
signal, process-level logging và thứ tự startup/shutdown; module không thực hiện
network I/O tại import time.
...
class HealthServer(uvicorn.Server):
    ...
    Telemetry entrypoint cần giữ ownership duy nhất của SIGINT/SIGTERM để dừng
    ...
    def capture_signals(self) -> Generator[None, None, None]:
        ...
        Yields:
            Quyền điều khiển cho ``uvicorn.Server.serve()`` trong khi entrypoint
...
class RuntimeState:
    ...
    Attributes:
        ...
        stopping: Entrypoint đã bắt đầu graceful shutdown.
    """
...
def create_health_app(runtime_state: RuntimeState) -> FastAPI:
    ...
    Args:
        runtime_state: State do entrypoint sở hữu và endpoint chỉ đọc.
...
async def serve_health_server(health_server: HealthServer) -> None:
    ...
    chuyển trạng thái đó thành ``RuntimeError`` để entrypoint cleanup nhất quán.
