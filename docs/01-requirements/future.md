...
### 26. Remote Start/Stop phiên sạc qua OCPP 2.0.1

- **Mô tả ngắn**: Cho phép backend gửi `RequestStartTransaction` và
  `RequestStopTransaction` tới trụ, theo dõi acceptance, timeout và kết quả
  `TransactionEvent`.
- **Tác dụng/Vai trò trong hệ thống**: Điều khiển phiên từ Admin/app, liên kết
  request với id token, EVSE và transaction thực tế, đồng thời xử lý race condition
  khi trụ mất kết nối.
- **Lý do hoãn lại**: MVP hiện chỉ quản lý trụ và giám sát phiên do trụ phát sinh;
  command từ xa cần contract authorization, id token, timeout, idempotency và audit
  chưa được chốt.
- **Liên quan đến planner/feature**: `backend-charging-stations.md`,
  `backend-charging-sessions.md` (AD-03, S-02).
- **Ngày ghi nhận**: 2026-07-31
- **Ghi chú thêm**: OCPP 2.0.1 dùng RequestStart/StopTransaction; không tái sử dụng
  trực tiếp contract RemoteStart/StopTransaction của OCPP 1.6.

---

### 27. Tính tiền phiên sạc

- **Mô tả ngắn**: Chốt giá tiền từ điện năng hợp lệ của phiên, biểu giá và các
  thành phần phí sau khi session hoàn tất.
- **Tác dụng/Vai trò trong hệ thống**: Tạo khoản phải thu có thể đối soát, giải
  thích được công thức và giữ snapshot biểu giá áp dụng tại thời điểm sạc.
- **Lý do hoãn lại**: Chưa có `docs/business-rules-billing.md` và chưa chốt biểu
  giá, thuế, làm tròn, timezone, xử lý meter reset hoặc session thiếu dữ liệu.
- **Liên quan đến planner/feature**: `backend-charging-sessions.md`, mục 4.4 trong
  `feature-list.md`.
- **Ngày ghi nhận**: 2026-07-31
- **Ghi chú thêm**: Billing nên thuộc domain `billing`; chỉ đọc session đã
  completed qua public service, không thêm trạng thái tiền vào state machine vận
  hành của `charging_sessions`.

---

### 28. Thanh toán và đối soát phiên sạc

- **Mô tả ngắn**: Tạo giao dịch thanh toán, nhận webhook cổng thanh toán, retry,
  quản lý pending payment/công nợ và đối soát.
- **Tác dụng/Vai trò trong hệ thống**: Thu tiền an toàn, cung cấp lịch sử thanh
  toán và kiểm soát các phiên quá hạn.
- **Lý do hoãn lại**: Chưa chọn payment gateway, chưa có tokenization/webhook
  security, idempotency key, refund và contract công nợ.
- **Liên quan đến planner/feature**: mục 2.3, 2.4, 3.6 và 4.11 trong
  `feature-list.md`; phụ thuộc hạng mục tính tiền.
- **Ngày ghi nhận**: 2026-07-31
- **Ghi chú thêm**: Thuộc domain `billing`; không lưu thông tin thẻ thô và không
  gộp payment status vào operational status của phiên sạc.
